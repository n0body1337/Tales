...
// Polyglot opening book — reads the embedded `ph-tal2.bin` file.
//
...
/// Minimum weight threshold as a percentage of the max weight (0–100).
/// Moves below (max_weight * BOOK_FILTER / 100) are filtered out.
const BOOK_FILTER: i32 = 20;

// ============================================================================
// Polyglot Zobrist random table — canonical 781-entry table.
// Indices: pieces [0..767], castling [768..771], en passant [772..779], turn [780].
// ============================================================================
...
/// This uses the standard 781-entry PG table, NOT the engine's internal Zobrist hash.
fn polyglot_key(pos: &Position) -> u64 {
    let mut key: u64 = 0;

    // Pieces: PG index = kind * 64 + square
    // Polyglot kind: BP=0, WP=1, BN=2, WN=3, BB=4, WB=5, BR=6, WR=7, BQ=8, WQ=9, BK=10, WK=11
    for sq in 0..64 {
        let pc = pos.pc[sq];
        if pc == NO_PC {
            continue;
        }
        // Map Tales piece to Polyglot kind index:
        // Tales: WP=0,BP=1,WN=2,BN=3,WB=4,BB=5,WR=6,BR=7,WQ=8,BQ=9,WK=10,BK=11
        // PG:    BP=0,WP=1,BN=2,WN=3,BB=4,WB=5,BR=6,WR=7,BQ=8,WQ=9,BK=10,WK=11
        // Mapping: PG_kind = tales_piece XOR 1 (swaps color bit)
        let pg_kind = (pc as usize) ^ 1;
        // PG square: rank*8 + file — Tales already uses this convention
        let pg_sq = (rank_of(sq as i32) * 8 + file_of(sq as i32)) as usize;
        key ^= PG[64 * pg_kind + pg_sq];
    }
    ...
    // En passant file
    if pos.ep_sq != NO_SQ {
        key ^= PG[772 + file_of(pos.ep_sq) as usize];
    }
...
fn polyglot_move_to_string(raw: u16, pos: &Position) -> String {
    let to_file = (raw & 7) as i32;
    let to_rank = ((raw >> 3) & 7) as i32;
    let from_file = ((raw >> 6) & 7) as i32;
    let from_rank = ((raw >> 9) & 7) as i32;
    let promotion = (raw >> 12) & 7;

    let mut from_sq = sq(from_file, from_rank);
    let mut to_sq = sq(to_file, to_rank);

    // Castling correction: Polyglot encodes castling as king → rook,
    // but we need king → destination (e.g. e1h1 → e1g1).
    if from_sq == E1 && to_sq == H1 && pos.king_sq[WC.index()] == E1 {
        to_sq = G1;
    }
    if from_sq == E1 && to_sq == A1 && pos.king_sq[WC.index()] == E1 {
        to_sq = C1;
    }
    if from_sq == E8 && to_sq == H8 && pos.king_sq[BC.index()] == E8 {
        to_sq = G8;
    }
    if from_sq == E8 && to_sq == A8 && pos.king_sq[BC.index()] == E8 {
        to_sq = C8;
    }

    // Prevent bogus squares from corrupted data
    from_sq = from_sq.clamp(0, 63);
    to_sq = to_sq.clamp(0, 63);

    let f1 = (b'a' + file_of(from_sq) as u8) as char;
    let r1 = (b'1' + rank_of(from_sq) as u8) as char;
    let f2 = (b'a' + file_of(to_sq) as u8) as char;
    let r2 = (b'1' + rank_of(to_sq) as u8) as char;

    let mut s = format!("{f1}{r1}{f2}{r2}");
...
#[inline]
fn simple_random(state: &mut u64, n: i32) -> i32 {
    *state ^= *state << 13;
    *state ^= *state >> 7;
    *state ^= *state << 17;
    ((*state >> 1) as i32) % n
...
#[inline]
fn is_infrequent(val: i32, max_weight: i32) -> bool {
    if max_weight > 2 && val < 2 {
        return true;
    }
...
/// Probe the Polyglot book for the given position.
/// Returns a weighted-random legal move, or None.
///
/// This is always called (not gated by UseBook) — the engine unconditionally
/// uses the embedded book for the first N plies of each game.
pub fn probe(pos: &Position, verbose: bool) -> Option<Move> {
    let count = entry_count();
    if count == 0 {
        return None;
    }
    ...
    while i < count {
        ...
        let mv_str = polyglot_move_to_string(raw_mv, pos);
    ...
    let weight_sum: i32 = candidates
        .iter()
        .filter(|(_, w)| !is_infrequent(*w, max_weight))
        .map(|(_, w)| *w)
        .sum();
    ...
    for (mv, w) in &candidates {
        ...
        if !is_infrequent(*w, max_weight) {
            vals_acc += *w;
            if simple_random(&mut rng_state, vals_acc) < *w {
                choice = Some(*mv);
            }
        }
    }
